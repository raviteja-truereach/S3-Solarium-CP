...
const NotificationTestContent: React.FC = () => {
  const { data, error, isLoading, isError, isSuccess, refetch } =
    useGetNotificationsQuery();

  console.log('🔍 Minimal Store Query State:', {
    isLoading,
    isError,
    isSuccess,
    hasData: !!data,
  ...
  return (
    <ScrollView style={styles.container}>
      <Text style={styles.title}>Minimal Store Test</Text>
...
// Main component with its own Provider
const MinimalStoreTest: React.FC = () => {
  return (
...
const styles = StyleSheet.create({
  ...
  dataContainer: { padding: 15, backgroundColor: '#e8f5e8', borderRadius: 5 },
  dataTitle: { fontWeight: 'bold', marginBottom: 10, fontSize: 16 },
  notifItem: {
    padding: 10,
    borderBottomWidth: 1,
...
export default MinimalStoreTest;
