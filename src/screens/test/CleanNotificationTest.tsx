...
const CleanNotificationTest: React.FC = () => {
  ...
  return (
    <ScrollView style={styles.container}>
      <Text style={styles.title}>Clean Notification Test</Text>
        ...
        <View style={styles.dataContainer}>
          ...
          {data.data?.notifications?.map((notif: any, index: number) => (
            <View key={index} style={styles.notifItem}>
              ...
              <Text style={styles.notifType}>Type: {notif.type}</Text>
            </View>
          ))}
        </View>
      )}
...
const styles = StyleSheet.create({
  ...
  notifType: { fontSize: 11, color: '#666' },
});

export default CleanNotificationTest;
